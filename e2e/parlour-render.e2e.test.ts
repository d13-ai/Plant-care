/**
 * The parlour page, rendered directly from a fixture. Offline: what matters
...
import { HANDLE, parlourPage, renderParlour, type Parlour } from "../api/parlour";
import { page } from "../api/tag";

const fixture: Parlour = {
...
test("it shows the keeper and every plant they have on show", () => {
  const html = renderParlour(fixture);
...
test("a plant with no photo still gets a tile", () => {
  const html = renderParlour({ ...fixture, plants: [{ ...fixture.plants[1] }] });
  expect(html).toContain("Thai Cutting");
  expect(html).toContain("noshot");
});

test("an empty parlour says so rather than looking broken", () => {
  const html = renderParlour({ ...fixture, plants: [] });
...
test("it is meant to be found and shared", async () => {
  ...
  const { html } = page("Amanda · PlantParlour", renderParlour(fixture), 200, '<meta property="og:title" content="x">');
...
test("a handle that could never exist is a 404 without asking the server", async () => {
  for (const bad of ["", "no", "has space", "a".repeat(21), "../../etc", "admin'--"]) {
    const { status } = await parlourPage(bad);
