-- A care entry logged by mistake (the greenhouse card's water drop is one
-- tap) can be removed, and the removal has to reach every other phone the
-- same way a plant's does: a deleted_at the pull sees. The tag page stops
-- showing removed entries.
alter table public.care_events add column if not exists deleted_at timestamptz;
