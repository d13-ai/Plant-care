...
// tag page's shell so the two public pages are visibly the same product, and
// it carries the description and og: tags a shared link wants; the app's own
// sign-in screen is inside the bundle and can do neither.
//
// The app itself stays at /, which is the sign-in screen for anyone without
// an account. Every button here goes there: signing in and signing up are the
// same button.
...
const FEATURES: Feature[] = [
  { icon: "leaf", title: "Identification and care", body: "Photograph the whole plant and a close-up or two; they get read together in one scan, and the care guide is written for the plant in front of you." },
  ...
  { icon: "cloud", title: "On every phone you sign into", body: "The greenhouse belongs to you, not to the phone you entered it on — so a new phone is a sign-in, not a fresh start." },
];
...
<section>
  <h2 class="caps">What it does</h2>
  ${grid(FEATURES)}
  <p class="note small">
    <strong>Straight about this:</strong> a parlour belongs to a keeper, so it starts with an
    account — an email and a 6-digit code, or one tap with Google. No passwords, and the same
    button whether you have one or not. Plants kept in a browser can't follow you to a new phone
    or be handed to a buyer, which is most of the point. A photo taken where there is no signal
    still saves, and uploads the next time there is.
  </p>
</section>
...
<div class="cta">
  <a class="btn" href="/">Start your parlour</a>
  <span class="small muted">Already have one? Same button.</span>
</div>
